...
import { SiDjango, SiTailwindcss, SiMysql, SiPostgresql, SiSqlite, SiPhp, SiArduino, SiCisco, SiVite } from 'react-icons/si';
...
const Skills = () => {
  ...
  // Handle tab click and scroll to center
  const handleTabClick = (tab) => {
    setActiveTab(tab);
    if (tabRefs.current[tab]) {
      tabRefs.current[tab].scrollIntoView({
        behavior: 'smooth',
        inline: 'center', // Ensures the selected tab is centered
        block: 'nearest',
      });
    }
  };
  ...
  return (
    <section id="skills" className="pt-4 bg-gradient-to-b from-gray-100 to-gray-300 w-full min-h-screen flex items-center justify-center">
      <div className="max-w-screen-lg mx-auto p-4 flex flex-col justify-center w-full h-full">
        ...
        {/* Tabs - Scrollable & Center on Click */}
        <div className="flex justify-center mb-8 px-4">
          <div className="flex overflow-x-auto whitespace-nowrap space-x-4 scrollbar-hide px-4">
            {tabs.map((tab) => (
              <button
                key={tab}
